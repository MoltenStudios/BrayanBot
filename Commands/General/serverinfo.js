    ...
    Utils.setupMessage(
      {
        ...
        variables: [
          ...
          ...Utils.userVariables(
            Utils.parseUser(interaction.guild.ownerId, interaction.guild),
            "guild-owner"
          ),
          ...Utils.botVariables(bot),
          ...guildVariables,
  ...
  return [
    ...
    },
    {
      ...
      replaceWith: guild.members.cache.filter(
        (member) =>
          member.presence && (member.presence.status !== "offline").size
      ),
    },
    {
    ...
    },
    {
      ...
      replaceWith: guild.members.cache.filter(
        (member) =>
          member.presence && member.presence.status == "dnd" && !member.user.bot
      ).size,
    },
    {
