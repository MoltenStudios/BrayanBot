...
module.exports.runSlash = async (bot, interaction) => {
    let guildVariables = await module.exports.fetchGuildVariables(interaction.guild, 'guild')
...
module.exports.fetchGuildVariables = async (guild, prefix = 'guild') => {
    ...
    return [
        ...
        { searchFor: new RegExp(`{${prefix}-online-members}`), replaceWith: guild.members.cache.filter(member => member.presence && (member.presence.status !== "offline").size) },
        { searchFor: new RegExp(`{${prefix}-online-bots}`), replaceWith: guild.members.cache.filter(member => member.presence && member.presence.status !== "offline" && member.user.bot).size },
        ...
        { searchFor: new RegExp(`{${prefix}-online-humans}`), replaceWith: guild.members.cache.filter(member => member.presence && member.presence.status == "online" && !member.user.bot).size },
        { searchFor: new RegExp(`{${prefix}-idle-humans}`), replaceWith: guild.members.cache.filter(member => member.presence && member.presence.status == "idle" && !member.user.bot).size },
        { searchFor: new RegExp(`{${prefix}-dnd-humans}`), replaceWith: guild.members.cache.filter(member => member.presence && member.presence.status == "dnd" && !member.user.bot).size },
        { searchFor: new RegExp(`{${prefix}-offline-humans}`), replaceWith: guild.members.cache.filter(member => member.presence && member.presence.status == "offline" && !member.user.bot).size },
    ]
}