const installModules = async () => {
    ...
    let package = require('./package.json'),
        ...
        nodeModules = "",
        install = (name) => {
            console.log(`\x1b[34m[Module Installer]`, `\x1b[37mInstalling ${`\x1b[1m${nodeModulesArray.length}\x1b[0m`}\x1b[37m modules, Please wait while we install modules. This may take few minutes.`)
            return new Promise(async (resolve, reject) => {
                childProcess.exec(`npm i ${name} --save`, async (err, stdout) => {
                    if (err) {
                        reject(err)
                    } else {
                        resolve(stdout)
...
installModules().then(() => {
    try {
        ...
        if (require('fs').existsSync('node_modules')) {
            console.log(`\x1b[34m[Module Installer]`, `\x1b[37mModules installed, please restart bot.`)
            process.exit(0)
        } else {
