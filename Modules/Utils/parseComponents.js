        ...
        if (variables && Array.isArray(variables) && variables[0]) {
          variables.forEach((variable, i) => {
            ...
            if (component.Placeholder)
              component.Placeholder = component.Placeholder.replace(
                variable.searchFor,
                variable.replaceWith
              );
          ...
          switch (component.Type.toLowerCase()) {
            case "button": {
              if (
                ...
                validButtonStyles.includes(component.Style.toLowerCase())
              ) {
                if (component.Style.toLowerCase() == "link") {
                  ...
                  if (!component.CustomID) {
                    ...
                    let button = new MessageButton().setCustomId(
                      component.CustomID
                    );
                    if (component.Style.toLowerCase() == "random") {
                      ...
                      ["primary", "blurple", 1, "1"].includes(
                        component.Style.toLowerCase()
                      )
                    ) {
                      button.setStyle("PRIMARY");
                    } else if (
                      ["secondary", "grey", 2, "2"].includes(
                        component.Style.toLowerCase()
                      )
                    ) {
                      button.setStyle("SECONDARY");
                    } else if (
                      ["success", "green", 3, "3"].includes(
                        component.Style.toLowerCase()
                      )
                    ) {
            ...
            case "selectmenu": {
              if (!component.CustomID) {
                ...
                Utils.logError(
                  `[Utils] [parseComponents] SelectMenu at least needs 1 option to work.`
                );
              } else {
