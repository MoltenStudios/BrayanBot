    ...
    set: (command) => {
        try {
            ...
            if (!enableWhenDisabled.includes(command.name.toLowerCase())) {
                ...
                command.commandData.Aliases.forEach((alias) => client.Aliases.set(alias, command.name));
                if (command.commandData.SlashCommand && command.commandData.SlashCommand.Enabled) {
                    ...
                    if (slashD && slashD.name && slashD.description) { client.SlashCmdsData.push(slashD); client.SlashCmds.push(command); }
                }
            }
        } catch (e) {
