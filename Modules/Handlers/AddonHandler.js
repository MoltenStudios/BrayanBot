...
module.exports = {
  ...
  getYAMLConfig: (name, config) => {
    if (
      ...
      return YAML.parse(
        fs.readFileSync(`./Addon_Configs/${name}/${config}.yml`, "utf-8")
      );
    } else {
      return false;
    }
                ...
                try {
                  ...
                  if (addon && typeof addon.run == "function") {
                    ...
                    if (
                      ...
                      typeof addon._customConfigs == "object"
                    ) {
                      ...
                      customConfig = Utils.createMultipleConfigs(
                        addon._customConfigs,
                        addonName
                      );
                    }
                    ...
                    if (typeof addon._log == "string" && !addon._author) {
                      console.log(
                        chalk.hex("#007bff").bold("[INFO] ") + addon._log
                      );
                    } else if (addon._log && typeof addon._author == "string") {
                      console.log(
                        `${chalk
                          .hex(addon._author.color || "#007bff")
                          .bold(
                            `[${addon._author ? addon._author : "[INFO]"}]`
                          )} ${chalk.bold(
                          addon._name ? addon._name : file.replace(".js", "")
                        )} addon loaded`
                      );
                      ...
                      console.log(
                        `${chalk
                          .hex(addon._author.color || "#007bff")
                          .bold(`[${addon._author.name}]`)} ${chalk.bold(
                          addon._name ? addon._name : file.replace(".js", "")
                        )} addon loaded`
                      );
                    }
                    ...
                    Utils.logWarning(
                      `Unable to execute ${
                        addon._name ? addon._name : file.replace(".js", "")
                      } addon ${addon._author ? `by ${addon._author}` : ""}`
                    );
                  }
                } catch (e) {
                  console.log(e);
