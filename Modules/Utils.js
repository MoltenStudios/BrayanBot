...
module.exports = {
  ...
  userVariables: (user, prefix) => {
    if (!user) {
      module.exports.logError(
        `[Utils] [userVariables] Invalid input for ${chalk.bold("user")}.`
      );
      return [];
    } else {
      return [
  ...
  parseUserFromMessage: (message, argument, messageMember = true) => {
    if (!message)
      return module.exports.logError(
        `[Utils] [parseUserFromMessage] Invalid input ${chalk.bold("message")}.`
      );
    if (!argument)
      return module.exports.logError(
    ...
    options.forEach((c) => {
      ...
      if (["STRING", "NUMBER", "INTEGER", "BOOLEAN"].includes(type)) {
        ...
        return module.exports.logWarning(
          "[Utils] [parseSlashArgs] Invalid Choice Type. Type: " + type
        );
      }
    });
    return args;
