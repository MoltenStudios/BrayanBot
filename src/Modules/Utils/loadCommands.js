import { BryanBot } from "../Structures/BryanBot.js";
import { Routes } from "discord.js";

/** @param {BryanBot} manager */
const loadCommands = async (manager, force = false) => {
  const enabledCMDS = manager.commands.filter((cmd) =>
    Array.isArray(cmd.commandData.Arguments)
  );

  const cmdsToRegister =
    force || process.argv.includes("--reset-slashCommands")
      ? manager.slashCommands.filter(
          (cmd) => !!enabledCMDS.find((c) => c.commandData.Name == cmd.name)
        )
