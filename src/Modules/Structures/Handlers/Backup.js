...
export class BackupHandler {
    ...
    async initialize(dirFiles) {
        ...
        if (process.argv.includes("--devmode")) {
            ...
            setInterval(() => {
                this.takeBackup().then(([result, output]) => {
                    console.log(output)
                    if (result) this.manager.logger.debug(`${prefix} Backup completed successfully. ${chalk.green(path.basename(output.path))}`);
                    else this.manager.logger.debug(prefix, "Backup failed.");
                }).catch((err) => (err.code === 'ENOENT' ? Utils.logger.warn(err) : Utils.logger.error(err)))
            }, ms("12h"))
        }
    ...
    async takeBackup() {
        return new Promise(async (resolve, reject) => {
            const output = fs.createWriteStream(path.join(this.backupDir, `backup-${Date.now()}.zip`));
            ...
            this.filePathsToBackup.forEach((filePath) => {
                ...
                if (fileInfo.isDirectory()) archive.directory(filePath, path.basename(filePath));
                else if (fileInfo.isFile()) archive.file(filePath, { name: path.basename(filePath) })
            });
