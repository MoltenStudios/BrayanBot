...
module.exports = async (bot, message) => {
    ...
    if(message.content.startsWith(config.Settings.Prefix)) {
        let msgArray = message.content.split(" "), command = msgArray[0].toLowerCase(), 
            args = msgArray.slice(1), hasPermission = false
        ...
        if(commands) {
            if(commands.commandData.Permission) {
                if(typeof commands.commandData.Permission == 'string')  commands.commandData.Permission = [commands.Permission]
                if(!commands.commandData.Permission[0]) commands.commandData.Permission[0] = "@everyone"
                if(commands.commandData.Permission.includes("@everyone") || commands.commandData.Permission.includes("everyone")) {
                    ...
                    for (const role of commands.commandData.Permission) {
                        if (!Utils.hasRole(message.member, role, true)) if(!hasPermission == true) hasPermission = false
                        else hasPermission = true
                    }
                }
            }
            if(hasPermission) commands.run(bot, message, args, config) 
            else message.reply({ 
                embeds: [
                    Utils.setupEmbed({
